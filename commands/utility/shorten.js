...
module.exports = class SayCommand extends Command {
    ...
    async run(msg) {
        ...
        mysqlConnection.query(`select * from url where longUrl='${message[1]}'`, function (error, results, fields) {
            if (error) throw (error);
            else if (!results[0]) { //if it is a new url to be shortend
                googleUrl.shorten(message[1], function (err, shortUrl) {
                    if (err) console.log(err)
                    else {
                        mysqlConnection.query(`insert into url (userId, longUrl, shortUrl, callTime) values (${msg.author.id}, '${message[1]}', '${shortUrl}', ${Date.now()})`)
